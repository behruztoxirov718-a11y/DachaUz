...
import { SlidersHorizontal, MapPin, Users, Bed, Star, X, Loader2 } from 'lucide-react';
import { useDachalar } from '../hooks/useDachalar';
import { districts, amenityLabels } from '../data/dachalar';
  ...
  return (
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-green-100 to-green-200">
        ...
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-2 py-1 text-sm font-semibold text-yellow-600 flex items-center gap-1">
          <Star size={13} fill="currentColor" /> {dacha.rating}
        </div>
        {!dacha.isAvailable && (
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between border-t pt-3">
          ...
          <div className="flex gap-3 text-sm text-gray-500">
            <span className="flex items-center gap-1"><Users size={13} />{dacha.guests}</span>
            <span className="flex items-center gap-1"><Bed size={13} />{dacha.beds}</span>
          </div>
        </div>
      </div>
    </div>
...
const Catalog = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [dachalar, district, guests, priceMax, selectedAmenities, sortBy, showOnlyAvailable]);
  ...
  const activeFilterCount = [district, guests, priceMax, showOnlyAvailable ? '1' : ''].filter(Boolean).length + selectedAmenities.length;
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between gap-3 mb-5">
          <button
            ...
            className="flex items-center gap-2 border border-gray-300 bg-white text-gray-700 font-medium px-4 py-2.5 rounded-xl hover:border-green-500 hover:text-green-700 transition-colors"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
              ...
              <div>
                ...
                <select value={priceMax} onChange={(e) => setPriceMax(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500 bg-gray-50">
                  <option value="">Muhim emas</option>
                  <option value="500000">500 000 som</option>
                  <option value="800000">800 000 som</option>
                  <option value="1000000">1 000 000 som</option>
                  <option value="1500000">1 500 000 som</option>
                </select>
              </div>
            </div>
            <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {district && (
              <span className="flex items-center gap-1 bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full">
                ...
                <button onClick={() => setDistrict('')} className="ml-1 hover:text-red-500"><X size={13} /></button>
              </span>
            )}
            {guests && (
              <span className="flex items-center gap-1 bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full">
                <Users size={13} /> {guests}+ kishi
                <button onClick={() => setGuests('')} className="ml-1 hover:text-red-500"><X size={13} /></button>
              </span>
            )}
          </div>
        )}
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((dacha) => <DachaCard key={dacha.id} dacha={dacha} />)}
          </div>
        ) : (
          <div className="text-center py-24">
            <SlidersHorizontal size={64} className="mx-auto mb-4 text-gray-200" strokeWidth={1} />
            <h3 className="text-xl font-bold text-gray-700 mb-2">Dacha topilmadi</h3>
            <p className="text-gray-400 mb-6">Filter shartlarini ozgartirib koring</p>
            <button onClick={clearFilters} className="bg-green-700 text-white font-semibold px-6 py-2.5 rounded-xl hover:bg-green-600 transition-colors">
              Filterni tozalash
            </button>
          </div>
        )}
